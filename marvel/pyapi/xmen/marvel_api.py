...
def marvelcharcall(stampystamp, hashyhash, pkeyz, lookmeup):
    ...
    pprint(XAVIER + "?name=" + lookmeup + "&ts=" + stampystamp + "&apikey=" + pkeyz + "&hash=" + hashyhash)
    char_data = []
    results = dict(r.json()).get('data').get('results')
    for result in results:
        char = {'name': result.get('name'), 'description': result.get('description'),
                'thumbnail': result.get('thumbnail').get('path') + result.get('thumbnail').get('extension')}
