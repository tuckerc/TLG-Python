...
import sys # used to read CL args
import os # used to walk the system
import fnmatch # for regex pattern matching
import pprint # for printing results in readable format
...
def find(pattern, path):
    ...
    for root, dirs, files in os.walk(path, topdown=True):
        ...
        for name in files: # always perform the nested loop, but it maybe empty
            if fnmatch.fnmatch(name, str(pattern).lower()): # if match
                result.append(os.path.join(root, name)) # add to our list
    return result # return the list

def main():
    """runtime code"""
    lookfor = sys.argv[1] # input("What pattern am I looking for (Example: *.txt or *.cfg) ")
    lookwhere = sys.argv[2] #input("What is the path in which I should search? ")
    results = find(lookfor, lookwhere)
    pprint.pprint(f"Results: {results}") # call function
